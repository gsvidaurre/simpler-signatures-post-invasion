...
In previous work<a href='#References'><sup>[1]</sup></a>, and an independent analysis of hierarchical mapping, we used known repeatedly sampled individuals for training supervised machine learning models. Here, we did not use these individuals for model training, but we did use these birds to visually validate the different measurements used as predictors for stochastic gradient boosting and random forests models. See the exploratory visuals generated below (AM1 - 7).
```{r echo = TRUE, eval = TRUE}
...
Drop calls with the suffix "_site_scale", as these were a single call per repeatedly sampled individual included at the site scale for hierarchical mapping analysis. Also drop the 4 NAT-UM4 calls that were inadvertently retained at the site scale in previous analysis (but did not change results reported)<a href='#References'><sup>[1]</sup></a>. We used the remaining 1561 calls for supervised machine learning analyses.
```{r echo = TRUE, eval = TRUE}
...
![Variable importance per model](/home/gsvidaurre/Desktop/GitHub_repos/vocal-learning-invasion/Code/images/AM10_VarImp.jpeg)
...
# Another version of the figure above for talks
# I tried making filled contours but 1) cannot put two scale fill calls on the same plot, and 2) it's nice to see overlap between the two distributions 
ggplot(data = mds_rf, aes(x = X, y = Y)) +
  geom_density2d(
    data = mds_rf %>%
      filter(range == "Native") %>%
  ...
  scale_color_manual(values = cols) +
  xlab("Multidimensional Scaling Dimension 1") + ylab(" MDS Dimension 2") + 
  theme_bw() +
  theme(
    legend.position = "top",
    axis.title = element_text(size = 16),
...
ggsave(file.path(gpath, "KernelDensity_RFAcousticSpace_sm.tiff"), units = "in", width = 5.84, height = 4.64, dpi = 300) 
...
![partial dependence plots, page 1](/home/gsvidaurre/Desktop/GitHub_repos/vocal-learning-invasion/Code/images/PDP_RF_01.jpeg)

### AM2.15

![partial dependence plots, page 2](/home/gsvidaurre/Desktop/GitHub_repos/vocal-learning-invasion/Code/images/PDP_RF_02.jpeg)
